...
function getWsUrl(httpUrl: string) {
  const configured =
    umbraNetwork === "mainnet"
      ? (process.env.HELIUS_MAINNET_WS_URL ??
        process.env.SOLANA_MAINNET_WS_URL ??
...
export async function getDevnetUmbraClient() {
  ...
  return getUmbraClient({
    ...
    ...(indexerApiEndpoint !== undefined ? { indexerApiEndpoint } : {}),
...
export function getDevnetUsdcMintOrThrow() {
  ...
  if (!mint) {
    throw new Error(
      `Missing ${umbraNetwork.toUpperCase()} USDC mint configuration`,
    );
  }
...
export function printUmbraError(error: unknown) {
  ...
  console.error(
    `${details?.name ?? "UmbraError"}: ${details?.message ?? String(error)}`,
  );
  ...
  if (logs) {
    ...
    if (logs.some((log) => log.includes("InstructionFallbackNotFound"))) {
      ...
      console.error(
        "  The latest published SDK built an instruction that the deployed devnet program rejected.",
      );
      console.error(
        "  This is an Umbra SDK/program deployment mismatch, not a wallet balance or signing issue.",
      );
    }
    ...
    if (logs.some((log) => log.includes("Program is not deployed"))) {
      ...
      console.error("Known Umbra devnet issue:");
      console.error(
        "  This SDK version points at an Umbra devnet program that is not currently deployed.",
      );
    }
  }
}
